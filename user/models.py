...
class CustomUser(AbstractUser):
    ...
    #ip = ArrayField(models.CharField(max_length=200), blank=True, null=True)
    #city = ArrayField(models.CharField(max_length=200), blank=True, null=True)
    #org = ArrayField(models.CharField(max_length=200), blank=True, null=True)

    def save(self, *args, **kwargs):
        is_new = self._state.adding
        super(CustomUser, self).save(*args, **kwargs)
        if is_new:
            #TODO: Доделать функцию сэйва
            users_in_group = CustomUser.objects.filter(groups__name=self.group)
            for user in users_in_group:
                LessonFeedback.objects.create(lesson=self, user=user)
        else:
